...
=======
  ...
  const CambiarEstado = async (id, nuevoEstado, deshabilitarCampo) => {
    try {
      const response = await axios.put(`${API_URL}/orders/${id}`, { Status: nuevoEstado });
      ...
      if (response.status === 200) {
        ...
        setPedidos((prevPedidos) =>
          prevPedidos.map((pedido) =>
            pedido.OrderId === id
              ? { ...pedido, Status: nuevoEstado, [deshabilitarCampo]: true }
              : pedido
          )
        );
        ...
        <table className="table table-striped table-hover">
          <thead className="table-dark">
            <tr>
              ...
              <th style={{ width: "15%" }}>Listo para entregar</th>
              ...
              <React.Fragment key={pedido.OrderId}>
                <tr className="hover-row">
                  ...
                  <td>
                    <button
                      className="btn btn-sm btn-outline-info"
                      onClick={() => setExpandedPedido(expandedPedido === pedido.OrderId ? null : pedido.OrderId)}
                    >
                      {expandedPedido === pedido.OrderId ? "Ocultar detalles" : "Ver detalles"}
                    </button>
                  </td>
                  <td>
                    <button
                      className="btn btn-sm btn-outline-primary"
                      onClick={() => CambiarEstado(pedido.OrderId, "Listo para entregar", "ListoDeshabilitar")}
                      disabled={pedido.ListoDeshabilitar}
                    >
                      Listo para entregar
                    </button>
                  </td>
                  <td>
                    <button
                      className="btn btn-sm btn-outline-success"
                      onClick={() => CambiarEstado(pedido.OrderId, "Entregado", "EntregadoDeshabilitar")}
                      disabled={pedido.EntregadoDeshabilitar}
                    >
                      Entregado
                    </button>
                  </td>
                ...
                {expandedPedido === pedido.OrderId && (
                  <tr>
                    <td colSpan="8" style={{ padding: "10px 20px", backgroundColor: "#f9f9f9" }}>
                      <h4>Productos del Pedido</h4>
                      {pedido.products && pedido.products.length > 0 ? (
